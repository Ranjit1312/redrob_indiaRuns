"""
rescore.py — v4 rules from the full-profile audit + final blend.
Base-rate-validated changes only:
...
Recomputes fit (v3 formula + B5), blends with saved LGBM (a=0.2), gates, writes
...
def main():
    ...
    # India, non-target city, unwilling to relocate (encoded as 0.40 by v3):
    # hybrid Tue/Thu office makes this a real blocker -> 0.33
    ...
    PROBES = {"CAND_0046525": "keep #1", "CAND_0061257": "Noida -> top10",
              "CAND_0041669": "Noida -> top10", "CAND_0052328": "Pune -> promote",
              "CAND_0062247": "promote", "CAND_0039383": "promote",
              "CAND_0005260": "promote", "CAND_0007411": "dormant -> out",
              "CAND_0061339": "audit-remove (la<signup)", "CAND_0015528": "audit-remove",
              "CAND_0066999": "concurrent-deg", "CAND_0064326": "career<edu (noise)",
              "CAND_0009691": "120d notice -> demote", "CAND_0071974": "Vizag no-reloc",
              "CAND_0086151": "demote"}
    print("[v4] probes:")
    for pid, note in PROBES.items():
        print(f"  {pid}: {int(ranks[pid]):6}  ({note})")
    ...
    old = pd.read_parquet(os.path.join(ART, "scores_blended.parquet"))["final"]
    ...
    print(f"[v4] top-50 overlap with previous blend: {len(o50 & n50)}/50; "
          f"wall={time.time()-t0:.0f}s; wrote submission.csv + top100candidates.jsonl")
